...
import {isArray,isString,isReactComponent,isObject,isFunction, isNumber, } from "@util/helpers";
import CommonBtn from "@components/bottom-btn";
...
class CreateForm extends PureComponent {
  ...
  componentDidMount() {
    const { type, detailUrl, } = this.props;
    this.getRequestData();
    if (type === 'edit' && detailUrl) {
      this.getDetailInfo();
    }
  }
  getDetailInfo = async () => {
    ...
    if (isString(detailUrl)) {
      const { data, code, } = await window.Get(url);
      if (code === 0) {
        this.setState({
          detailInfo: detailFormat(data),
        }, this.initFormValue)
      }
    }
  };
  ...
  submitData = async (val) => {
    const {type,submitUrl,submitFormat, match,} = this.props;
    const id = _.get(match,'params.id', '');
    const {detailInfo,} = this.state;
    this.setState({
      loading: false,
    ...
    if (type === "edit") {
      params['id'] = detailInfo.id || id;
    }
    const {code,message:msg,} = await window.Post(submitUrl,params);
    if (code) {
      this.setState({
        loading: false,
      });
...
CreateForm.propTypes = {
  ...
  detailUrl: PropTypes.string, // 获取详情地址
  submitFormat: PropTypes.func, // 提交的时候格式化数据
  detailFormat: PropTypes.func,// 获取详情后格式化数据
};
