...
export default class YearPicker extends PureComponent {
  ...
  changeValue = (e) => {
    ...
    const year = e === null ? null : window.moment(e).format("YYYY");
    this.setState({
      year: e,
      showYear: false,
    });
  ...
  changeYear = (e) => {
    const { onChange, } = this.props;
    const year = window.moment(e).format("YYYY");
    this.setState({
      year,
      showYear: false,
    });
  ...
  static formatData(val) {
    if (val) {
      ...
      if (isMoment) {
        ...
        return match && match[0] ? window.moment(`${match[0]}-01-01 00:00:00`) : null;
      }
    }
    return val;
  }
